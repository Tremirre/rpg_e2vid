...
if __name__ == "__main__":
    ...
    output_name = os.path.basename(args.bag).split(".")[
        0
    ]  # /path/to/mybag.bag -> mybag
            ...
            for topic, msg, t in bag.read_messages():
                if topic == args.event_topic:
                    ...
                    if (
                        ...
                        or event_msg_sum >= total_num_event_msgs - 1
                    ):
                        print(
                            "Event messages: {} / {}".format(
                                event_msg_sum + 1, total_num_event_msgs
                            )
                        )
                    event_msg_sum += 1
    ...
    if not args.no_zip:
        ...
        path_to_events_zipfile = os.path.join(
            args.output_folder, "{}.zip".format(output_name)
        )
        with zipfile.ZipFile(path_to_events_zipfile, "w") as zip_file:
            zip_file.write(
                path_to_events_file,
                basename(path_to_events_file),
